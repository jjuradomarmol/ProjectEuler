...
function getSumOfMaxRoute(startingRow, startingCol) {
  var sumOfMaxRoutes = [];

  function calcSumOfMaxRoute(startingRow, startingCol) {
    ...
    var leftPath = calcSumOfMaxRoute(startingRow + 1, startingCol);
    var rightPath = 0;
    if (startingCol < triangle[startingRow + 1].length - 1) {
      rightPath = calcSumOfMaxRoute(startingRow + 1, startingCol + 1);
    }
  ...
  console.log(
    `The maximum total from top to bottom of the triangle is ${calcSumOfMaxRoute(
      startingRow,
      startingCol
    )}`
  );
